...
table.ah { border-collapse:collapse; font-size:0.82rem; width:100%; margin-top:1rem; }
table.ah th { background:var(--accent); color:#fff; padding:6px 10px; text-align:left; }
table.ah td { padding:5px 10px; border-bottom:1px solid var(--border); }
table.ah tr:nth-child(even) td { background:#f7f8fa; }
...
<?php include 'nav.php'; ?>
<div class="container">
  <h1>Alternating Harmonic Series</h1>
  <p class="subtitle">
    Each format accumulates $\ln 2 = \sum_{k=1}^{\infty} \frac{(-1)^{k+1}}{k}
    = 1 - \tfrac{1}{2} + \tfrac{1}{3} - \tfrac{1}{4} + \cdots$
    by computing in-format additions and subtractions left to right.
    This series converges, but slowly (like $O(1/k)$), and the running sum
    always stays in $(0, 1)$, so there is no overflow.
    The oscillation around $\ln 2$ damps as the terms shrink.
    Stagnation occurs when $1/k$ falls below the format's ULP at the current sum —
    at that point the sum is frozen at its best-achievable approximation to $\ln 2 \approx 0.6931$.
    Unlike the plain harmonic series (which grows without bound), the alternating version
    tests how well each format handles <em>cancellation</em>: repeated addition of
    small opposite-sign quantities near a moderate value.
  </p>
  ...
  <div class="card">
    ...
    <div class="plot-wrap">
      <canvas id="cv-altharm" width="760" height="380" style="max-width:100%"></canvas>
    </div>
    <div class="legend-row" id="ah-legend">
      <!-- populated by JS -->
    </div>
  </div>
  ...
  <div class="card" style="margin-top:1rem">
    ...
    <p class="format-note" style="margin-top:0.5rem">
      $k^*$ is the number of terms accumulated before the format's resolution is exhausted.
      Compare with the plain harmonic series: there the sum diverges and the format permanently
      misses all tail contributions; here the frozen sum is the format's best approximation to
      the finite limit $\ln 2$.
    </p>
  </div>
</div>
...
// Alternating harmonic series computation
var TRUE_LN2 = Math.LN2;  // 0.693147...

function altHarmonicSeries(fmt, N) {
  var sums = new Float64Array(N);
  var stagnAt = null;
  var sum = 0, prev = 0;
  for (var k = 1; k <= N; k++) {
    var term = (k % 2 === 1) ? 1 / k : -1 / k;
...
function renderLegend() {
  var leg = document.getElementById('ah-legend');
  var html = '<span><span class="lswatch-ref"></span>true ln 2</span>';
...
//Draw 
function draw(N) {
  var series = currentConfig.map(function(cfg) { return altHarmonicSeries(cfg.fmt, N); });
  ...
  var canvas = document.getElementById('cv-altharm');
  ...
  // True ln(2) reference line (red dashed)
  ctx.strokeStyle = '#c8303a'; ctx.lineWidth = 1.5; ctx.setLineDash([6, 4]);
  ctx.beginPath();
  ctx.moveTo(PAD.l, yPx(TRUE_LN2));
  ctx.lineTo(PAD.l + pw, yPx(TRUE_LN2));
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillStyle = '#c8303a'; ctx.font = 'italic 10px sans-serif'; ctx.textAlign = 'left';
  ctx.fillText('ln 2 ≈ 0.6931', PAD.l + 4, yPx(TRUE_LN2) - 4);
  ...
  ctx.fillText('partial sum  Σ (−1)ᵏ⁺¹/k', 0, 0);
  ctx.restore();

  // Summary table 
  var tbody = document.getElementById('ah-tbody');
  tbody.innerHTML = '';
  ...
  currentConfig.forEach(function(cfg, ci) {
    ...
    var absErr = Math.abs(frozen - TRUE_LN2);
    var relErr = absErr / TRUE_LN2;
    ...
    tr.appendChild(td(TRUE_LN2.toPrecision(5)));
